...
export interface ForecastDay { date: string; maxTemperature: number; severity: HeatwaveSeverity; model: string }
export interface StationReading { station: string; location: string; region: Region; latitude: number; longitude: number; latestReading: number; humidity: number; windSpeed: number; regionalForecast: number; validation: 'consistent' | 'watch' | 'divergent'; quality: 'good' | 'delayed' | 'suspect'; provider: string; timestamp: string }
