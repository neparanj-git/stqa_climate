...
export interface Forecast { date:string; maxTemperature:number; lowerBound:number; upperBound:number; heatwaveProbability:number; severity:Severity; model:string }
export interface Station { station:string; location:string; region:Region; latestReading:number; humidity:number; windSpeed:number; regionalForecast:number; validation:'consistent'|'watch'|'divergent'; quality:'good'|'delayed'|'suspect'; timestamp:string }
...
export interface Snapshot { generatedAt:string; connection:'live'; forecastProvider:string; regions:Summary[]; hottest:Summary; activeAlerts:number; reportingStations:number; totalStations:number; dataQuality:number }
