...
async function refresh(){
  ...
  const regions=REGIONS.map(region=>{const readings=stations.filter(station=>station.region===region);const hottest=readings.reduce((a,b)=>a.regionalForecast>b.regionalForecast?a:b);const severity=classifySeverity(hottest.regionalForecast,regionMeta[region].normal);return{region,latestTemp:hottest.regionalForecast,severity,normal:regionMeta[region].normal,latitude:hottest.latitude,longitude:hottest.longitude,departure:Number((hottest.regionalForecast-regionMeta[region].normal).toFixed(1))}});
