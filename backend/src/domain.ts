...
export interface StationReading { station: string; location: string; region: Region; latitude: number; longitude: number; latestReading: number; humidity: number; windSpeed: number; regionalForecast: number; validation: 'consistent' | 'watch' | 'divergent'; quality: DataQuality; provider: string; timestamp: string }
export interface ForecastPoint { date: string; maxTemperature: number; severity: HeatwaveSeverity; model: string }
