import { observations as seedObservations, stations as seedStations, regionMeta } from './data.ts';
...
export class ClimateRepository {
  private observations: Observation[] = [...seedObservations];
  private stations: StationReading[] = [...seedStations];
  ...
  latestForRegion(region: Region) { return this.findObservations({ region }).at(-1)!; }
  ingest(reading: IngestReading) {
    const regionalForecast = this.latestForRegion(reading.region).maxTemperature;
